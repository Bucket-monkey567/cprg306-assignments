        ...
        <section className="flex flex-col items-center bg-white rounded-2xl my-2">
            {/* Name input */}
                <section className="flex flex-row">
                    <input className="m-2.5 p-2 border-2 border-blue-400 rounded-md text-black" type="text" placeholder="Item Name" value={item.name} onChange={(e) => setItem({...item, name:e.target.value})} />
                    <select className="m-2.5 p-2 border-2 border-blue-400 rounded-md bg-white text-black" value={item.category} onChange={(e) => setItem({...item, category:e.target.value})}>
